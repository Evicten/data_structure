...
from ae_state_evolution_new import run_one_alpha
...
DAMPING = get_float_env("SE_DAMPING", 0.9)
...
M0_U = get_float_env("SE_M0_U", 0.01)
M0_V = get_float_env("SE_M0_V", 0.01)
Q0   = get_float_env("SE_Q0", 1.0)
V0   = get_float_env("SE_V0", 0.5)
...
def main():
    ...
    for a in ALPHA_VALUES:
        ...
        # Save (root only)
        if rank == 0:
            print("state_traj last:", [shp(z) for z in state_traj[-1]])
            print("state_traj first:", [shp(z) for z in state_traj[0]])
