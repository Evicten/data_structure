...
def hat_vars_MCMC(alpha, beta, q_vars, samples, gamma, delta):
    ...
    h_star_centered = (h_star - 1/np.sqrt(q)*Xi@m)@h_star_covariance # (samples, 2)
    ...
    #V_hat = alpha*np.mean(prox**2+1/V*(prox-omega)*(h_star_centered/q@h_star_covariance@m.T - (prox-omega)*Xi/np.sqrt(q))) #need to change here the first component for other activations
    V_hat = alpha*np.mean(np.maximum(prox, 0)**2+1/V*(prox-omega)*(h_star_centered/q@h_star_covariance@m.T - (prox-omega)*Xi/np.sqrt(q)))
