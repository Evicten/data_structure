import numpy as np
from amp_denoising_functions_vectorized import integral_parameters, f_out, df_out, f_Q, bmv, bmm, bouter, compute_M_N_from_V_c1, beta_tilde, c_1, c_2, G_out
from mpi4py import MPI
from state_evolution import generate_latents

def update_mean(X, w, Gamma_out):
    n , d = X.shape
    omega = X @ w / np.sqrt(d) - Gamma_out

    return omega

def update_Gamma_out(Gout, omega, y):
    """
    Gout: (s, s)
    omega: (n, s)
    y (n, )
    
    returns Gamma_out (n, s)
    """
    return omega @ Gout.T * (y == 1)[: , None]


def update_w(X, Gamma_out, w, Gout, alpha): 
    return X.T @ Gamma_out / np.sqrt(d) - alpha/2 * w @ Gout.T


def channel(beta_u, beta_v, gamma): 

    bt = beta_tilde(beta_v)
    Gout = G_out(beta_u, beta_v, gamma, bt)
